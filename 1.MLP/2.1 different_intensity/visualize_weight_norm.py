...
check_name = f'./checkpoint{str(noise_intensity)}/'
pic_save_dir = f'weight_plots{noise_intensity}'
...
def plot_param_distribution_combined(global_stats_dict, save_fig=False):
    ...
    fig.suptitle(f'Parameter Distribution (Noise {noise_intensity})', fontsize=20)
...
def plot_norm_trend(l1_norm_all, l2_norm_all, all_norm_all, save_fig=False):
    ...
    fig.suptitle(f'L1 Norm Trend (Noise {noise_intensity})', fontsize=18)
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(18, 18))
    fig.suptitle(f'L2 Norm Trend (Noise {noise_intensity})', fontsize=18)
...
def plot_train_test_loss(save_fig=False):
    ...
    # 图表美化
    ax.set_title(f'Training & Testing Loss Curve (Noise Intensity {noise_intensity})', fontsize=16)
