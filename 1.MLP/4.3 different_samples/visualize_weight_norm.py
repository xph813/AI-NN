...
from diff_stage_rand_seed import noise_intensity, check_name, num_points

# ===================== 配置项（与训练代码对齐） =====================
# noise_intensity = 0.1  # 需与训练代码的噪声强度一致
# check_name = f'./checkpoint_uniform{str(noise_intensity)}/'
pic_save_dir = f'weight_plots_num_points{str(num_points)}'
...
def plot_param_distribution_combined(global_stats_dict, save_fig=False):
    ...
    fig.suptitle(f'Parameter Distribution (num_points: {num_points})', fontsize=20)
...
def plot_norm_trend(l1_norm_all, l2_norm_all, all_norm_all, save_fig=False):
    ...
    fig.suptitle(f'L1 Norm Trend (num_points: {num_points})', fontsize=18)
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(18, 18))
    fig.suptitle(f'L2 Norm Trend (num_points: {num_points})', fontsize=18)
...
def plot_train_test_loss(save_fig=False):
    ...
    # 图表美化
    ax.set_title(f'Training & Testing Loss Curve (num_points: {num_points})', fontsize=16)
