...
class PeriodicMLP(nn.Module):
    def __init__(self, hidden_units=64):
        ...
        # 输入层接收2维周期特征（sin(x)+cos(x)），而非原始1维x
        self.hidden1 = nn.Linear(2, hidden_units)  
        self.hidden2 = nn.Linear(hidden_units, hidden_units)
        self.output = nn.Linear(hidden_units, 1)
    
    def forward(self, x):
        # 核心：注入周期先验 → 将x转换为sin(x)+cos(x)（周期2π的正交基）
        x_period = torch.cat([torch.sin(x), torch.cos(x)], dim=1)  # 形状(N,2)
        
        # 后续层保持tanh激活（匹配sinx值域）
        x = torch.tanh(self.hidden1(x_period))
...
model = PeriodicMLP(hidden_units = 64)
...
check_name = f'./checkpoint_1phase{str(num_points)}_prior/'
